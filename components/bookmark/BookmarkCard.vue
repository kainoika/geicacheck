<template>
    <div :class="[
        ...
        currentBookmark.visited
            ? 'bg-green-50 border-green-200 shadow-green-100 border-l-4 border-l-green-500'
        ...
        <div class="relative h-48 bg-gray-100 group">
            <img v-if="circle.circleCutImageUrl" :src="circle.circleCutImageUrl" :alt="`${circle.circleName}のサークルカット`"
                :class="[
                    'w-full h-full object-cover cursor-pointer transition-all duration-200 hover:opacity-90',
                    currentBookmark.visited ? 'opacity-80' : 'opacity-100'
                ]"
        ...
        <div class="p-4">
            ...
            <div class="mb-3">
                <button
                    ...
                    :class="[
                        'flex items-center gap-1 px-3 py-1.5 rounded-md text-sm font-medium transition-colors w-full justify-center',
                        currentBookmark.visited
...
interface Emits {
    ...
    (e: 'toggle-visited', circleId: string): void
}
...
// Computed
const circle = computed(() => props.bookmark.circle)
const currentBookmark = computed(() => {
    return getBookmarkByCircleId(props.bookmark.circleId) || props.bookmark
})
const bookmarkCategory = computed(() => {
    return currentBookmark.value?.category
})
...
const handleBookmark = (category: BookmarkCategory) => {
    emit('bookmark', props.bookmark.circleId, category)
}

const handleToggleVisited = () => {
    emit('toggle-visited', props.bookmark.circleId)
}
