import { OGPData, DEFAULT_OGP, CircleData, EventData } from "../types/ogp";
 ...
 * @returns OGPデータ
 */
export function generateOGPData(
  circle: CircleData,
  event: EventData,
  circleId: string
): OGPData {
  ...
  // 画像URL生成
  const image = circle.circleCutImageUrl || DEFAULT_OGP.image || '';
  ...
  return {
    ...
    image,
    url,
    siteName: DEFAULT_OGP.siteName || 'ゲイカチェック',
    type: DEFAULT_OGP.type || 'website'
  };
}
 ...
 * @returns HTML文字列
 */
export function generateOGPHTML(ogpData: OGPData, circleId: string): string {
  // HTMLエスケープ関数
 ...
 * @returns エラー用HTML文字列
 */
export function generateErrorHTML(error: string, circleId?: string): string {
  const escapeHtml = (unsafe: string): string => {
    return unsafe
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
