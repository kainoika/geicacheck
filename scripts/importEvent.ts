...
const GEICA33_EVENT = {
  ...
  venue: {
    name: "TRC東京流通センター第二展示場1階Eホール",
    address: "東京都大田区平和島6丁目1−1 東京流通センター センタービル 1F",
    accessInfo: "東京モノレール「東京流通センター」駅より徒歩約1分",
  },
  description: "アイカツ！シリーズオンリーイベント「芸能人はカードが命！33」",
  status: "upcoming",
  isDefault: true,
  mapData: "",
 ...
 * イベントデータを作成（手動ID）
 */
async function createGeikaEvent() {
  ...
  try {
    ...
    await setDoc(eventRef, GEICA33_EVENT);
...
async function main() {
  ...
  console.log(
    "⚠️ 本番データを変更します。実行前にバックアップを取ってください。"
  );

  try {
    await createGeikaEvent();
