...
function convertCircleData(original: OriginalCircleData): ConvertedCircleData {
  return {
    ...
    circleImageUrl: original.circleImageUrl || "",
    genre: parseGenre(original.genre),
    placement: {
      block: original.block,
      number1: original.number1,
      number2: original.number2 || "",
    },
    ...
    ownerId:
      original.ownerId && original.ownerId.trim() !== ""
...
async function convertData() {
  try {
    ...
    // 元のJSONファイルを読み込み
    const inputPath = join(process.cwd(), "data", "geica33-circle.json");
    const outputPath = join(
      process.cwd(),
      "data",
      "geica33-circle-converted.json"
    );
    ...
    // データ変換
    const convertedData: ConvertedCircleData[] =
      originalData.map(convertCircleData);
