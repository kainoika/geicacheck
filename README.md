...
**重要**: 初回実行前に[セットアップ手順](scripts/MIGRATION_SETUP.md)に従ってサービスアカウントキーを取得してください。
...
**セットアップ済みの場合**: そのまま実行可能です。Firestoreのセキュリティルールを変更する必要はありません。

**メリット**:
- ✅ セキュリティルールの手動変更が不要
- ✅ Firebase Admin SDKで安全に実行
- ✅ サービスアカウント認証による高いセキュリティ

詳細は [`scripts/MIGRATION_SETUP.md`](scripts/MIGRATION_SETUP.md) を参照してください。
